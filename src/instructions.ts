...
export function createUnlockInstruction(
  ...
  const data = Buffer.concat([Buffer.from(Int8Array.from([2]).buffer), Buffer.concat(seeds)])
...
export function createChangeDestinationInstruction(
  ...
  const data = Buffer.concat([Buffer.from(Int8Array.from([3]).buffer), Buffer.concat(seeds)])
